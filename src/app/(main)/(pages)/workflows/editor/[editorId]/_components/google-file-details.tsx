              ...
              <CardContent className="flex flex-wrap gap-2 p-4">
                {(
                  [
                    ...
                    "name",
                    "mimeType",
                    title === "Google Drive" ? "id" : undefined,
                  ] as Array<keyof typeof file>
                )
                  .filter(
                    (detail): detail is keyof typeof file =>
                      detail !== undefined && detail in file
                  )
                  .map((detail) => (
                    <div
                      key={String(detail)}
                      onClick={() =>
                        onAddTemplate(nodeConnection, title, file[detail])
                      }
                      className="flex cursor-pointer gap-2 rounded-full bg-white px-3 py-1 text-gray-500 border"
                    >
                      <span className="flex bg-black text-white rounded-xl py-1 px-2 items-center justify-between">
                        {String(detail)}:
                      </span>
